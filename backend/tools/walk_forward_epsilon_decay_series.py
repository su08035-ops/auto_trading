"""Run walk-forward epsilon-decay experiments from the batch-32 candidate."""
...
def fold_config(base_config, *, epsilon_decay, validation_start, split_date, end_date):
...
def aggregate_rows(fold_rows):
    ...
    return {
        ...
        "epsilon_decay": frame["epsilon_decay"].iloc[0],
...
def write_markdown(root_dir, fold_rows, aggregate):
    decay = aggregate["epsilon_decay"]
    lines = [
        f"# {aggregate['experiment']} epsilon_decay={decay}",
        "",
...
def write_combined_summary(results_root, aggregate_rows):
    lines = [
        "# Epsilon-decay walk-forward comparison",
        "",
        "| Experiment | Epsilon Decay | Mean Agent Val | Mean B&H Val | Median Agent Val | Worst Agent Val | Mean Val Sharpe | Mean Agent Turnover | Mean Val Return Std | Mean Best Ep | 2026 Agent Test | 2026 B&H Test | 2026 Agent Turnover |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    (results_root / "실험24-25_epsdecay_walk_forward_summary.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def main():
    ...
    parser.add_argument("--epsilon-decays", nargs="+", type=float, default=[0.9995, 0.9999])
    ...
    all_aggregates = []
    for offset, decay in enumerate(args.epsilon_decays):
        experiment = f"실험{next_number + offset}_epsdecay-{slug_value(decay)}"
        ...
        for fold, validation_start, split_date in FOLDS:
            ...
            print(
                f"Running epsilon_decay={decay} {fold}: validation={validation_start}..{split_date}, test starts={split_date}",
                flush=True,
            )
            config = fold_config(
                base_config,
    ...
    pd.DataFrame(all_aggregates).to_csv(
        results_root / "실험24-25_epsdecay_walk_forward_aggregate.csv", index=False
    )
