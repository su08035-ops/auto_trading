"""Run multi-seed walk-forward validation for the hidden-256 candidate."""
...
def fold_config(
    ...
    *,
    ...
    config["action"]["values"] = action_values
    config["agent"].update(
        algorithm=algorithm,
        hidden_dim=256,
        loss="huber",
        ...
        rows.append(
            {
                ...
                "mean_validation_return_std": group["validation_return_std"].mean(),
...
def overall_aggregate(seed_rows):
    ...
    return {
        ...
        "mean_agent_validation_sharpe": frame["mean_agent_validation_sharpe"].mean(),
        ...
        "mean_validation_return_std": frame["mean_validation_return_std"].mean(),
...
def write_markdown(
    root_dir, seed_rows, aggregate, *, experiment, algorithm, action_values
):
    lines = [
        f"# {experiment} {algorithm} hidden-256 multi-seed walk-forward",
        "",
        f"Candidate: algorithm={algorithm}, action_values={action_values}, hidden_dim=256, batch_size=32, loss=huber, gamma=0.99, target_update_interval=500, epsilon_min=0.05, epsilon_decay=0.999.",
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    number(row["mean_agent_validation_sharpe"]),
        ...
        [
            ...
            + " | ".join(
                [
                    ...
                    number(aggregate["mean_agent_validation_sharpe"]),
...
def main():
    ...
    number = args.start_number or next_experiment_number(results_root)
    action_suffix = "action-5-" if len(args.action_values) == 5 else ""
    suffix = (
        f"hidden-256-{action_suffix}multiseed"
        if args.algorithm == "dqn"
        else f"double-dqn-hidden-256-{action_suffix}multiseed"
    )
    experiment = f"실험{number}_{suffix}"
