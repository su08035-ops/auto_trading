"""Run walk-forward hidden-dimension experiments from the batch-32 candidate."""
...
def fold_config(base_config, *, hidden_dim, validation_start, split_date, end_date):
...
def aggregate_rows(fold_rows):
    ...
    return {
        ...
        "hidden_dim": frame["hidden_dim"].iloc[0],
...
def write_markdown(root_dir, fold_rows, aggregate):
    hidden_dim = aggregate["hidden_dim"]
    lines = [
        f"# {aggregate['experiment']} hidden_dim={hidden_dim}",
        "",
...
def write_combined_summary(results_root, aggregate_rows):
    lines = [
        "# Hidden-dimension walk-forward comparison",
        "",
        "| Experiment | Hidden Dim | Mean Agent Val | Mean B&H Val | Median Agent Val | Worst Agent Val | Mean Val Sharpe | Mean Agent Turnover | Mean Val Return Std | Mean Best Ep | 2026 Agent Test | 2026 B&H Test | 2026 Agent Turnover |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    (results_root / "실험26-27_hidden_walk_forward_summary.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def main():
    ...
    parser.add_argument("--hidden-dims", nargs="+", type=int, default=[64, 256])
    ...
    all_aggregates = []
    for offset, hidden_dim in enumerate(args.hidden_dims):
        experiment = f"실험{next_number + offset}_hidden-{hidden_dim}"
        ...
        for fold, validation_start, split_date in FOLDS:
            ...
            print(
                f"Running hidden_dim={hidden_dim} {fold}: validation={validation_start}..{split_date}, test starts={split_date}",
                flush=True,
            )
            config = fold_config(
                base_config,
    ...
    pd.DataFrame(all_aggregates).to_csv(
        results_root / "실험26-27_hidden_walk_forward_aggregate.csv", index=False
    )
