"""Run walk-forward batch-size experiments as separate numbered runs."""
...
def fold_config(base_config, *, batch_size, validation_start, split_date, end_date):
...
def aggregate_rows(fold_rows):
    ...
    return {
        ...
        "batch_size": frame["batch_size"].iloc[0],
...
def write_markdown(root_dir, fold_rows, aggregate):
    lines = [
        f"# {aggregate['experiment']} batch_size={aggregate['batch_size']}",
        "",
...
def write_combined_summary(results_root, aggregate_rows):
    lines = [
        "# Batch-size walk-forward comparison",
        "",
        "| Experiment | Batch | Mean Agent Val | Mean B&H Val | Median Agent Val | Worst Agent Val | Mean Agent Turnover | Mean B&H Turnover | 2026 Agent Test | 2026 B&H Test | 2026 Agent Turnover |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    number(row["mean_bh_validation_turnover"]),
    ...
    (results_root / "실험19-20_batch_walk_forward_summary.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def main():
    ...
    parser.add_argument("--batch-sizes", nargs="+", type=int, default=[128, 32])
    ...
    all_aggregates = []
    for offset, batch_size in enumerate(args.batch_sizes):
        experiment = f"실험{next_number + offset}_batch-{batch_size}"
        ...
        for fold, validation_start, split_date in FOLDS:
            ...
            print(
                f"Running batch_size={batch_size} {fold}: validation={validation_start}..{split_date}, test starts={split_date}",
                flush=True,
            )
            config = fold_config(
                base_config,
    ...
    pd.DataFrame(all_aggregates).to_csv(
        results_root / "실험19-20_batch_walk_forward_aggregate.csv", index=False
    )
